...
test("终端输入精确命中 ll 时关闭行内提示，避免语义噪声", () => {
  const catalog = buildCommandCompletionCatalog({
    remoteCompletions: [
...
test("当前输入已精确命中命令时关闭行内补全，显式搜索仍保留其他前缀项", () => {
  const catalog = buildCommandCompletionCatalog({
    remoteCompletions: [
  ...
  assert.deepEqual(searchInlineCommandCompletions("clear", { completions: catalog }), []);
  ...
  assert.deepEqual(
    searchInlineCommandCompletions("clear_console", { completions: catalog }),
    [],
  );
});
