...
test("v0.4.0 发布版本在 Web、Rust 与 Tauri 元数据中保持一致", () => {
  ...
  assert.match(cargoToml, /^version = "0\.4\.0"$/m);
...
test("发布仍强制 updater 签名和 SHA-256 校验", () => {
  ...
  assert.match(workflow, /default: "v0\.4\.0"/);
  assert.match(workflow, /\$expectedVersion = "0\.4\.0"/);
