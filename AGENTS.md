...
- Version 0.3.1 remains a Windows-only Tauri 2 client. React, xterm and the selected IDE-style visual direction may remain in the WebView2 UI, but SSH, SFTP, monitoring, updates, transfers and durable state must run in the Rust core so hiding the window never pauses background work.
...
- Version 0.3.1 is explicitly authorized as a one-time release without Authenticode because no trusted code-signing certificate is available. Keep updater signatures and SHA-256 checks mandatory, disclose the unsigned publisher state in release notes, and do not generalize this exception to later versions.
